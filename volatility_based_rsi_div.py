...
def find_local_extrema(series, order=5, mode='max'):
    ...
    if mode == 'max':
        return pd.Series(series.index.isin(series.iloc[argrelextrema(series.values, np.greater, order=order)[0]].index), 
                         index=series.index).astype(int)
    elif mode == 'min':
        return pd.Series(series.index.isin(series.iloc[argrelextrema(series.values, np.less, order=order)[0]].index), 
                         index=series.index).astype(int)
    else:
...
def load_data(file_path):
    ...
    try:
        ...
        missing_columns = [col for col in required_columns if col not in df.columns and col.capitalize() not in df.columns]
...
class RSIDivergenceStrategy(Strategy):
    """RSI Divergence Trading Strategy with Volatility-Based Position Sizing"""
    # Strategy parameters
    ...
    def init(self):
        ...
        self.bearish_div = self.I(lambda: self.data.bearish_div)
    ...
    def next(self):
        ...
        if not self.position:  # If not in a position
            ...
            elif self.bearish_div[-1] == 1 and hasattr(self, 'sell'):
                # Enter short position with calculated size
                self.sell(size=size)
        ...
        elif hasattr(self.position, 'is_short') and self.position.is_short:  # If in a short position
            # Exit if stop loss or take profit is hit
...
def run_backtest(crypto, data_file):
    ...
    # Run backtest
    bt = Backtest(df, RSIDivergenceStrategy, cash=INITIAL_CASH, commission=COMMISSION_RATE)
    stats = bt.run()
    ...
    return {
        ...
        "Avg. Position Size": trades_df["Position Size"].mean() if len(trades_df) > 0 else 0
    }

# --- Main Function ---
def main():
    """Main function"""
    logging.info("Starting multi-cryptocurrency RSI divergence backtesting with position sizing...")
    ...
    if results:
        ...
        # Create position size vs. return plot
        plt.figure(figsize=(10, 6))
...
if __name__ == "__main__":
    # Import scipy here to avoid import error if not used
